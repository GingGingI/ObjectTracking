package c.gingdev.objecttracking.ml
...
import c.gingdev.objecttracking.common.Utils
...
internal constructor(private val context: Context,
    ...
    @Throws(FirebaseMLException::class)
    internal fun classifyAwait(image: ImageProxy, awaitMilliSeconds: Long=this.awaitMilliSeconds): FirebaseModelOutputs? {
        if (!initialized || interpreter==null) {
            initialize()
        }
        try {
    ...
    @Throws(FirebaseMLException::class)
    internal fun classifyAwait(image: FirebaseVisionImage, awaitMilliSeconds: Long=this.awaitMilliSeconds): FirebaseModelOutputs? {
        if (!initialized || interpreter==null) {
            initialize()
        }
        try{
